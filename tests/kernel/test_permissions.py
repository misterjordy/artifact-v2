...
from artiFACT.kernel.permissions.hierarchy import REQUIRED_ROLES
...
@pytest.mark.asyncio
async def test_contributor_cannot_approve(contributor_user: FcUser, node_uid: uuid.UUID) -> None:
    """Contributor cannot approve — requires subapprover or higher."""
    db = AsyncMock()

    with (
        patch("artiFACT.kernel.permissions.resolver.get_cached_role", return_value=None),
        patch("artiFACT.kernel.permissions.resolver.get_active_grants", return_value=[]),
        patch("artiFACT.kernel.permissions.resolver.get_ancestors", return_value=[node_uid]),
        patch("artiFACT.kernel.permissions.resolver.set_cached_role", new_callable=AsyncMock),
    ):
...
@pytest.mark.asyncio
async def test_node_grant_overrides_global_role(viewer_user: FcUser, node_uid: uuid.UUID) -> None:
    ...
    with (
        patch("artiFACT.kernel.permissions.resolver.get_cached_role", return_value=None),
        patch("artiFACT.kernel.permissions.resolver.get_active_grants", return_value=[grant]),
        patch("artiFACT.kernel.permissions.resolver.get_ancestors", return_value=[node_uid]),
        patch("artiFACT.kernel.permissions.resolver.set_cached_role", new_callable=AsyncMock),
    ):
...
@pytest.mark.asyncio
async def test_revoked_grant_not_honored(viewer_user: FcUser, node_uid: uuid.UUID) -> None:
