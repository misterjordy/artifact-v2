...
async def search(
    ...
    results = await search_facts(db, q, limit=limit, program_uids=program_uid)
    return {
...
async def sidebar_search_results(
    ...
    # Filter to accessible program UIDs
    accessible_uids = [str(p.node_uid) for p in accessible_programs]
    filter_uids = program_uid if program_uid else accessible_uids

    results = await search_facts(db, q, program_uids=filter_uids)

    # Build breadcrumb strings for grouping
    result_dicts = []
    for r in results:
        d = r.model_dump()
