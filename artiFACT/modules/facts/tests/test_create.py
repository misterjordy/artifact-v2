...
from artiFACT.kernel.models import FcFact, FcNode, FcNodePermission, FcUser
from artiFACT.modules.facts.service import create_fact


def _mock_can_for_role(user_role: str):
    """Return a mock can() that resolves based on user's global_role."""
...
async def test_create_sets_created_by(db: AsyncSession, admin_user, child_node):
    ...
    with patch("artiFACT.modules.facts.service.can", side_effect=_mock_can_for_role("admin")), \
         patch("artiFACT.modules.facts.versioning.can", side_effect=_mock_can_for_role("admin")), \
         patch("artiFACT.modules.facts.service.validate_duplicate", new_callable=AsyncMock), \
         patch("artiFACT.kernel.events.publish", new_callable=AsyncMock):
...
async def test_contributor_creates_proposed(db: AsyncSession, contributor_user, child_node):
    ...
    with patch("artiFACT.modules.facts.service.can", side_effect=_mock_can_for_role("contributor")), \
         patch("artiFACT.modules.facts.versioning.can", side_effect=_mock_can_for_role("contributor")), \
         patch("artiFACT.modules.facts.service.validate_duplicate", new_callable=AsyncMock), \
         patch("artiFACT.kernel.events.publish", new_callable=AsyncMock):
        fact, version = await create_fact(
...
async def test_approver_creates_published(db: AsyncSession, approver_user, child_node):
    ...
    with patch("artiFACT.modules.facts.service.can", side_effect=_mock_can_for_role("approver")), \
         patch("artiFACT.modules.facts.versioning.can", side_effect=_mock_can_for_role("approver")), \
         patch("artiFACT.modules.facts.service.validate_duplicate", new_callable=AsyncMock), \
         patch("artiFACT.kernel.events.publish", new_callable=AsyncMock):
        fact, version = await create_fact(
