...
async def get_descendants(db: AsyncSession, root_uid: uuid.UUID) -> list[uuid.UUID]:
    ...
    cte = (
        select(FcNode.node_uid).where(FcNode.node_uid == root_uid).cte(name="tree", recursive=True)
    )
    cte = cte.union_all(select(FcNode.node_uid).join(cte, FcNode.parent_node_uid == cte.c.node_uid))
